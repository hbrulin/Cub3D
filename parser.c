...
/*   Updated: 2020/01/18 14:06:00 by hbrulin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pos_from_map(t_env *env, int i, int j)
{
...
int		ft_parser(t_env *env)
{
	...
	if ((env->error = loop(env)) != SUCCESS)
		return (env->error);
	if ((env->map.k_total % (env->map.nb_line + 1) != 0) ||
		env->map.flag_n == 0)
		return (WRONG_MAP);
	return (SUCCESS);
}
